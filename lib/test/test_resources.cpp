...
BOOST_AUTO_TEST_CASE ( res_builtin )
{
    ...
    BOOST_CHECK_THROW ( const uint8_t i = res1[700], std::runtime_error );
...
BOOST_AUTO_TEST_CASE ( res_external )
{
    ...
    const std::string binpath ( framework::master_test_suite().argv[1] );
    const Resource res1 ( binpath, "override.res" );
...
BOOST_AUTO_TEST_CASE ( res_override )
{
    ...
    const std::string binpath ( framework::master_test_suite().argv[1] );
    const Resource res1 = LOAD_RESOURCE ( lib_histdist_cl, binpath, "override.res" );
